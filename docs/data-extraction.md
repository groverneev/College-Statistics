...
1. Inspect the target school's existing JSON and any school-specific extraction script.
2. Identify the blocking sections to extract rather than redoing the entire schema.
...
- Prefer `pypdf` for text and AcroForm extraction.
- Use `pdfplumber` when table structure matters, but do not assume table boundaries are reliable.
